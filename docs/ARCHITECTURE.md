...
`oj.starter_catalog` 内置 16 道经重新表述的洛谷基础题和 85 个独立测试点。数据库使用
`system_state` 保存 `luogu_starter_catalog_v1` 安装标记：首次启动只补充不存在的题号并写入
标记，后续启动不重复导入，也不会覆盖用户编辑或恢复用户主动删除的题目。测试环境通过
`OJ_SEED_CURATED_PROBLEMS=false` 保持各测试用例的数据隔离；管理员执行系统重置时保留安装
标记，符合“重置后题库为空”的验收语义。
...
| 提交与本人结果 | 允许 | 允许 |
| 他人提交结果、重新评测 | 禁止 | 允许 |
| 私有日志 | 本人仅见总分，`details` 为空 | 完整明细 |
| 公开日志 | 登录后允许 | 允许 |
| 用户列表与角色管理 | 禁止 | 允许 |
| AI 任务 | 仅本人 | 可查看全部 |

被封禁用户即使持有旧 Session 也返回 403；封禁操作同时清除该用户所有服务端 Session。
`public_cases=false` 时，提交者可访问日志但只能获得 `score`、`counts` 和空 `details`，其他
普通用户返回 403；`public_cases=true` 时所有登录用户可查看逐测试点状态、耗时和内存。管理员
始终可见完整明细。访问审计动作统一写为 `view_logs`。删除题目使用显式事务级联清理测试点结果、提交、访问
...
模型地址通过 `_completion_url` 统一处理：纯域名自动补 `/v1`，显式的 API 子路径原样保留，
最终追加 `/chat/completions`。单次请求默认超时由 90 秒提高到 300 秒；超时、HTTP、网络和
响应解析错误分别转换为不含密钥的诊断信息。Streamlit 任务控制台使用 fragment 定时重跑，
默认 2 秒轮询一次，只更新控制台区域；间隔可调，并展示最近刷新时间、输入/输出/总 Token、
费用与后端进度。中断按钮直接调用取消 API，后台任务在等待模型响应时也能收到取消信号。
