...
def build() -> None:
    ...
    story = [
        ...
        *screenshot("06-ai-iteration.png", "图 7  迭代意见与测试点配额同屏配置"),
        ...
        p("6. 测试与成果展示", "H1"),
        p(
            "自动测试共 29 项，覆盖用户与角色、认证优先级、题目 CRUD、限制继承、级联删除、测试点"
            "可见性、单题按角色提交范围、独立用户统计、动态 C 注册、语言命令校验、提交限流、筛选"
            "和重评、Python 的 AC/WA/RE/TLE、C++ CE、测试日志公开与审计、系统重置、AI 进度/取消/"
            "费用、洛谷页面解析与 Streamlit 浏览器冒烟。当前本机全部通过，后端语句覆盖率为 85.70%。"
        ),
        table(
            [
                ["检查", "结果"],
                ["pytest", "29 passed"],
                ["Coverage", "85.70%（门槛 80%）"],
        ...
        *screenshot("00-dashboard-admin.png", "图 8  等级、资源统计与角色化功能入口"),
        PageBreak(),
        p("7. AI 使用说明", "H1"),
        p(
